...
function createMetaNameTag(prop, val) {
	return (`<meta name="${prop}" content="${val}">\n`)
}
...
if (matterKeys.includes("og")) {
	if (frontMatter.og.description) head += (createMetaNameTag("description", frontMatter.og.description))
	...
	if (frontMatter.og.image.url) head += (createMetaTag("og:image", frontMatter.og.image.url))
	if (frontMatter.og.image.url) head += (createMetaTag("twitter:image", frontMatter.og.image.url))
